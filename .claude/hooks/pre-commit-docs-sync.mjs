 ...
 * ADR:  #0002 — fails open when .claude/config/doc-map.json is missing
 *       or has no entries. Activates automatically as the map grows
 *       with each phase.
 *
...
function pushUnique(list, item) {
  if (!list.some((x) => x.doc === item.doc && x.label === item.label))
    list.push(item);
}
...
function groupByDoc(items) {
  ...
  return [...byDoc.entries()].map(([doc, labels]) => ({
    doc,
    labels: [...labels],
  }));
}
