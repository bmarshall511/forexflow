 ...
 * commit land. The target scripts do not exist until Phase 2; this hook
 * detects that and no-ops cleanly.
 *
 * When all three scripts are present, any failure blocks the commit.
 */
...
async function main() {
  ...
  ].filter((s) => typeof scripts[s.name] === "string" && scripts[s.name].length > 0)

  if (toRun.length === 0) return allow()

  // If pnpm is not on PATH, fail open — the environment is not ready.
  const pnpmCheck = spawnSync("pnpm", ["--version"], { cwd: repoRoot, stdio: "ignore" })
