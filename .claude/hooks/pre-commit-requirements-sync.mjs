 ...
 *
 * Fails open if docs/requirements/ does not yet exist.
 */
...
async function main() {
  ...
  if (stagedFiles.some((p) => p.startsWith("docs/requirements/") && p.endsWith(".md"))) {
    return allow()
  }
  ...
  const sourceFiles = stagedFiles.filter((p) =>
...
function extractCommitMessage(cmd, repoRoot) {
  ...
  // -m "..." or --message="..."
  const m = cmd.match(/\s-(?:m|-message(?:=|\s+))(?:'([\s\S]*?)'|"([\s\S]*?)"|(\S+))/)
