 ...
 * placeholder commit mode is active.
 *
 * Fails open when docs/requirements/ does not exist yet (pre-scaffold).
 * Fails open on test and config files (other rules handle those).
 */
...
async function main() {
  ...
  if (/\.(test|spec|contract\.test|visual\.spec|bench)\.(ts|tsx)$/.test(relPath)) return allow()
  if (/\/(generated|__fixtures__|__tests__|__mocks__)\//.test(relPath)) return allow()
...
function extractExports(content) {
  const out = []
  const re = /^\s*export\s+(?:default\s+)?(?:async\s+)?(?:function|class|const|let|var|interface|type|enum)\s+([A-Za-z_][A-Za-z0-9_]*)/gm
  let m
