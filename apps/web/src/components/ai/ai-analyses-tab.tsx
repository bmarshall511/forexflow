...
import type { AiAnalysisData, AiAnalysisSections, TradeDetailData, PendingOrderData, OpenTradeData, ClosedTradeData, TradeSource, TradeDirection, OrderType, TradeCloseReason, TradeOutcome } from "@fxflow/types"
...
import { ChevronDown, ChevronLeft, ChevronRight, Bot, User, AlertTriangle, Loader2, Trash2, ExternalLink } from "lucide-react"
...
function analysisBadge(status: string): { label: string; className: string } {
  switch (status) {
    case "completed": return { label: "Done", className: "bg-emerald-500/15 text-emerald-600 border-emerald-500/30" }
    case "running": return { label: "Running", className: "bg-blue-500/15 text-blue-600 border-blue-500/30" }
    case "pending": return { label: "Pending", className: "bg-amber-500/15 text-amber-600 border-amber-500/30" }
    case "failed": return { label: "Failed", className: "bg-red-500/15 text-red-600 border-red-500/30" }
...
function triggeredByLabel(triggeredBy: string): string {
  switch (triggeredBy) {
    case "auto_fill": return "On Open"
    case "auto_close": return "On Closed"
    case "auto_pending": return "On Limit Placed"
    case "auto_interval": return "Interval"
...
function triggeredByBadgeClass(triggeredBy: string): string {
  ...
  switch (triggeredBy) {
    case "auto_fill": return "bg-purple-500/10 text-purple-600 border-purple-500/20"
    case "auto_close": return "bg-blue-500/10 text-blue-600 border-blue-500/20"
    case "auto_pending": return "bg-indigo-500/10 text-indigo-600 border-indigo-500/20"
    case "auto_interval": return "bg-violet-500/10 text-violet-600 border-violet-500/20"
    default: return "bg-purple-500/10 text-purple-600 border-purple-500/20"
  }
}

function tradeTitle(row: Pick<AnalysisSummary, "instrument" | "entryPrice" | "openedAt" | "tradeNotes">): string {
...
function WinProbBar({ value }: { value: number | null }) {
  ...
  const color =
    value >= 60 ? "bg-status-connected" : value >= 40 ? "bg-status-warning" : "bg-status-disconnected"
  ...
  return (
    <div className="flex items-center gap-1.5 min-w-[80px]">
      ...
      <span className="text-[10px] font-mono tabular-nums text-muted-foreground w-7 text-right">{value}%</span>
    </div>
  )
}
...
function QualityDots({ score }: { score: number | null }) {
  ...
  return (
    <div className="flex items-center gap-0.5" aria-label={`Quality ${score}/10`}>
      ...
      <span className="ml-1 text-[10px] font-mono tabular-nums text-muted-foreground">{score}</span>
    </div>
  )
}
...
function AnalysisDetailSheet({
  ...
  useEffect(() => {
    if (!summary) { setAnalysis(null); return }
    setIsLoading(true)
    fetch(`/api/ai/analyses/${summary.tradeId}/${summary.id}`)
      .then((r) => r.json())
      .then((j: { ok: boolean; data?: AiAnalysisData }) => { if (j.ok && j.data) setAnalysis(j.data) })
      .catch(() => {})
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/ai/analyses/${summary.tradeId}/${summary.id}`, { method: "DELETE" })
  ...
  return (
    <>
      <Sheet open={!!summary} onOpenChange={(open) => { if (!open) onClose() }}>
        <SheetContent side="right" className="w-full sm:max-w-xl overflow-y-auto p-0">
          <SheetHeader className="px-5 pt-5 pb-3 border-b border-border/50">
            <div className="flex items-center gap-2">
              ...
              {summary && (
                <span className={cn("text-xs font-medium", summary.direction === "long" ? "text-emerald-600" : "text-red-500")}>
                  {summary.direction === "long" ? "↑ Long" : "↓ Short"}
                </span>
              )}
              {summary && (
                <Badge variant="outline" className={cn("text-[10px] h-5", analysisBadge(summary.status).className)}>
            ...
            <SheetDescription className="text-xs flex items-center gap-3 flex-wrap">
              {summary && (
                <>
                  ...
                  <span>·</span>
                  <span>{modelLabel(summary.model)} · {summary.depth}</span>
                  <span>·</span>
      ...
      <TradeDetailDrawer
        trade={viewTrade}
        open={!!viewTrade}
        onOpenChange={(open) => { if (!open) setViewTrade(null) }}
      />
    </>
  )
}
...
function AnalysisCard({
  ...
  return (
    <div className={cn(
      ...
      {/* Primary row */}
      <div
        className="px-3 py-2 flex items-center gap-2 min-w-0 cursor-pointer rounded-lg"
        onClick={onClick}
        role="button"
        tabIndex={0}
      ...
      >
        ...
        {/* Model badge */}
        <Badge variant="outline" className={cn("text-[10px] h-5 shrink-0", modelBadgeColor(row.model))}>
          {modelLabel(row.model)}
        </Badge>

        {/* Source badge */}
        <Badge variant="outline" className={cn("text-[10px] h-5 gap-0.5 shrink-0 hidden sm:inline-flex", triggeredByBadgeClass(row.triggeredBy))}>
          {AUTO_TRIGGERED_VALUES.has(row.triggeredBy) ? <Bot className="size-2.5" /> : <User className="size-2.5" />}
        ...
        <button
          type="button"
          className="shrink-0 p-0.5 rounded hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        ...
        <div className="border-t border-border/50 px-3 py-2 space-y-2">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Win Prob</span>
            ...
            {/* Depth */}
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Depth</span>
              <Badge variant="outline" className={cn("text-[10px] h-5 capitalize", depthBadgeColor(row.depth))}>
                {row.depth}
              </Badge>
            </div>
            {/* Trade subtitle */}
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Trade</span>
              <p className="text-[11px] text-muted-foreground truncate">{tradeTitle(row)}</p>
            </div>
          </div>
        </div>
      )}
...
export function AiAnalysesTab() {
  ...
  const fetchData = useCallback(async (p: number, tb: string, st: string) => {
    ...
    try {
      ...
      const json = await res.json() as { ok: boolean; data?: { rows: AnalysisSummary[]; total: number } }
      if (json.ok && json.data) {
  ...
  useEffect(() => { void fetchData(page, triggeredBy, status) }, [fetchData, page, triggeredBy, status])
  useEffect(() => {
  ...
  return (
    <div className="space-y-3">
      ...
      {stuckRows.length > 0 && (
        <div className="flex items-center gap-2.5 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2">
          ...
          <p className="text-xs text-amber-700 flex-1">
            {stuckRows.length} analysis{stuckRows.length > 1 ? "es are" : " is"} stuck in &quot;Running&quot; — the daemon may have crashed.
          </p>
          <Button
            variant="outline"
            size="sm"
            className="h-7 text-xs border-amber-500/40 text-amber-700 hover:bg-amber-500/10 shrink-0"
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <Select value={triggeredBy} onValueChange={(v) => { setPage(1); setTriggeredBy(v) }}>
        ...
        <span className="text-xs text-muted-foreground">{total > 0 && `${total} total`}</span>
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="ml-auto h-8 text-xs gap-1.5 text-muted-foreground hover:text-destructive"
      ...
      {isLoading ? (
        ...
        <p className="py-10 text-center text-sm text-muted-foreground">No analyses found</p>
      ) : (
