...
export function ZoneControlsPopover({
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        align="end"
        className="max-h-[75vh] w-[320px] overflow-hidden rounded-xl p-0"
        sideOffset={6}
      >
            ...
            <div className="space-y-0.5">
              ...
              {settings.curve.enabled && (
                <InlineSelect
                  ...
                  value={
                    settings.curve.timeframe === chartTimeframe
                      ? "__chart__"
                      : (settings.curve.timeframe ?? "")
                  }
                  onChange={(v) => {
...
function MasterToggle({
  ...
  return (
    <div className="mx-3 mb-2 flex items-center gap-3 rounded-lg border p-2.5">
      <div className="min-w-0 flex-1">
        <label htmlFor={id} className="cursor-pointer text-sm font-medium leading-tight">
          {label}
        </label>
        <p className="text-muted-foreground mt-0.5 text-[10px] leading-snug">{description}</p>
      </div>
      <button
        id={id}
        type="button"
        ...
        className={cn(
          ...
          enabled ? (color === "emerald" ? "bg-emerald-500" : "bg-blue-500") : "bg-muted",
        )}
      >
        <span
          className={cn(
            "pointer-events-none ml-0.5 mt-0.5 block h-5 w-5 rounded-full bg-white shadow-md transition-transform",
...
function CompactToggle({
  ...
  return (
    ...
    >
      ...
      >
        {checked && (
          <svg className="text-background h-3 w-3" viewBox="0 0 12 12" fill="none">
            <path
      ...
      <span className="min-w-0 flex-1">
        ...
        {hint && (
          <span className="text-muted-foreground mt-px block text-[10px] leading-tight">
            {hint}
          </span>
        )}
      </span>
...
function InlineSelect({
  ...
  return (
    <div className="ml-6 flex items-center justify-between gap-2 px-2 py-1">
      <span className="text-muted-foreground text-[11px]">{label}</span>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="bg-background focus:ring-ring min-w-[5rem] rounded-md border px-2 py-0.5 text-right text-[11px] focus:outline-none focus:ring-1"
      >
        {options.map((o) => (
    ...
    <div className="space-y-1 py-1">
      ...
      <input
        ...
        className={cn(
          ...
          "[&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-sm",
          color === "emerald"
            ? "bg-emerald-500/20 accent-emerald-500 [&::-webkit-slider-thumb]:bg-emerald-500"
            : "bg-blue-500/20 accent-blue-500 [&::-webkit-slider-thumb]:bg-blue-500",
        )}
      />
...
function AdvancedSection({
  ...
  return (
    <Collapsible open={open} onOpenChange={onOpenChange}>
      ...
      <CollapsibleTrigger className="text-muted-foreground hover:text-foreground flex w-full items-center justify-between px-3 py-2 text-[11px] transition-colors">
