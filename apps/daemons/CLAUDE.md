...
- **Prompt caching (Phase 3)**: Tier 2 and Tier 3 calls in `scanner.ts` pass the system prompt as a cacheable content block with `cache_control: { type: "ephemeral" }`. Within a single scan cycle the second and later calls hit the cache, giving ~90% discount on input tokens for the stable parts of the prompt. `cache_read_input_tokens` and `cache_creation_input_tokens` are logged to the scan activity log when non-zero.
- **AI re-evaluation (Phase 3, opt-in)**: `ai-trader/re-evaluator.ts` wires the existing `buildReEvalPrompt()` from `prompt-builder.ts` (previously dead code) into the trade-manager's management cycle. Gated by four layers: `mgmt.reEvaluationEnabled` (default off), `mgmt.reEvaluationMode` (`"off" | "suggest" | "auto"`, default `"suggest"`), `mgmt.reEvaluationIntervalHours` per-trade (default 2), and `mgmt.reEvaluationGraceMinutes` after fill (default 15). Auto-execution additionally requires `confidence >= mgmt.reEvaluationMinConfidence` (default 75). Uses the shared `CostTracker` so re-eval costs can't escape the daily/monthly EdgeFinder budget. All decisions logged to `AiTraderOpportunity.managementLog` with action `"re_evaluate"` and a structured `[mode→disposition]` detail string.
- **Feedback-loop data (Phase 4a)**: Every `AiTraderOpportunity` row now carries `tier2Confidence`, `tier2Passed`, and `tier2DecidedAt` — Tier 2's own confidence/disposition/timestamp distinct from the final Tier 3 confidence. Populated on every Tier 2 write path (pass, fail, gate-block, invalid-JSON). Enables post-mortem queries like "how well-calibrated is Haiku's quick-filter vs Sonnet's final decision?" without re-parsing the stored `tier2Response` blob. Every scan also persists up to 20 near-miss signals to the new `AiTraderNearMiss` table (fire-and-forget, errors never block the scan cycle) so Tier 1 thresholds can be tuned from historical data instead of the in-memory ring buffer.
...
- **Multi-agent analysis (opt-in, default ON)**: When `AiTraderConfig.multiAgentEnabled` is true, Tier 3 becomes a multi-agent pipeline: 2 parallel Haiku analyst briefs (technical + macro/risk) → bull case (Haiku, argues FOR) → bear case (Haiku, argues AGAINST + counters bull) → Judge (Sonnet, weighs both sides + past reflections). `multi-agent.ts` orchestrates the flow. Debate data stored on `AiTraderOpportunity` (`technicalBrief`, `macroRiskBrief`, `bullCase`, `bearCase`, `debateCost`, `debateInputTokens`, `debateOutputTokens`). When disabled, falls back to existing single Sonnet call. Adds ~$0.001 per candidate.
- **Post-trade reflection (always on)**: `reflection-engine.ts` generates a Haiku reflection asynchronously when any EdgeFinder trade closes. Captures what the thesis was, what happened, and actionable lessons. Stored in `AiTraderReflection` table (one per closed opportunity, `@@unique(opportunityId)`). Future Judge prompts inject 2-3 relevant past reflections (same pair+profile, recency-weighted) as "experience memory". Fire-and-forget: errors never block trade close flow. Cancelled trades are skipped (no lesson to extract).
- **New scan log entry types**: `brief_complete`, `bull_case`, `bear_case`, `judge_decision` — broadcast via existing `ai_trader_scan_log_entry` WS message.
...
- **Feedback-loop context at placement (Phase 4a)**: `placeMarketEntry()` populates `SmartFlowTrade.atrAtPlacement` (ATR in price units used to compute SL/TP) and `SmartFlowTrade.regimeAtPlacement` (trending/ranging/volatile/low-volatility) at trade creation time. Regime is detected from the H1 candles already cached by `calculateAtr()` so there's zero extra network I/O. Enables queries like "how did SmartFlow trades perform in trending vs ranging regimes?" in post-mortem analytics.
...
- **Prompt caching (Phase 3)**: `ai-monitor.ts` passes its SYSTEM_PROMPT as a cacheable content block with `cache_control: { type: "ephemeral" }`. Cache hits on the identical system prompt give ~90% discount on input tokens for back-to-back evaluations across multiple trades, and log the cache token counts to console when non-zero.
