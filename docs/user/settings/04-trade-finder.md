...
If 3 auto-orders are already open, Trade Finder will wait until one closes before placing another.

### Max Daily Auto-Trades

The most auto-trades Trade Finder can place in a single day. Default is **5**.

Once this limit is reached, Trade Finder goes back to scan-only mode until the next day.
