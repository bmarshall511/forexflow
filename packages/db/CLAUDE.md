...
- **One live DB only**: `./data/fxflow.db` at the repo root. Both daemon and web `.env.local` point here via `DATABASE_URL=file:../../data/fxflow.db`. Nothing else should ever exist as a real SQLite file inside the repo — all other `.db` files are bugs.
