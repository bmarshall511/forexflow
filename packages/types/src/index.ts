...
export interface AiTraderConfigData {
  ...
  /** Enable multi-agent analysis (analyst briefs → bull/bear debate → judge).
   *  When false, falls back to single Sonnet Tier 3 call. */
...
export interface AiTraderOpportunityData {
  ...
  // Multi-agent debate data (when multiAgentEnabled)
  technicalBrief: string | null // Technical analyst brief
  macroRiskBrief: string | null // Macro/risk analyst brief
  bullCase: string | null // Bull advocate argument
  bearCase: string | null // Bear advocate counter-argument
  debateCost: number // Combined cost of briefs + bull + bear Haiku calls
  ...
  tier3Response: string | null // Deep analysis response (judge decision in multi-agent mode)
...
export interface AiTraderScanLogEntry {
  ...
  type:
    ...
    | "trade_rejected"
    | "gate_blocked"
    | "brief_complete" // Analyst briefs generated (multi-agent)
    | "bull_case" // Bull advocate argument generated
    | "bear_case" // Bear advocate counter-argument generated
    | "judge_decision" // Judge (Sonnet) final decision after debate
  message: string
  detail?: string
...
/** Post-trade reflection generated by AI — qualitative lessons from closed trades. */
export interface AiTraderReflectionData {
  id: string
  ...
  profile: AiTraderProfile
  confidence: number
  outcome: "win" | "loss" | "breakeven" // Cancelled trades don't get reflections
  realizedPL: number
  entryRationale: string | null
  reflection: string // Narrative of what happened (3-5 sentences)
  lessonsLearned: string // Actionable bullet points
  primaryTechnique: AiTraderTechnique | null
  regime: AiTraderMarketRegime | null
  session: AiTraderSession | null
  model: string | null
